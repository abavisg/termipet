use clap::{Parser, Subcommand};
use termipet::{
    adopt_pet, clean_pet, feed_pet, play_pet, potty_pet, reset_pet, run_shell, show_status,
    sleep_pet, train_pet, walk_pet,
};

#[derive(Parser)]
#[command(name = "termipet")]
#[command(about = "A virtual pet that lives in your terminal", long_about = None)]
#[command(version)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}

#[derive(Subcommand)]
enum Commands {
    /// Adopt a new pet
    Adopt {
        /// Name of your pet
        #[arg(short, long)]
        name: String,

        /// Species of your pet (e.g., dog, cat, dragon)
        #[arg(short, long)]
        species: String,
    },
    /// Check your pet's current status
    Status,
    /// Feed your pet to restore hunger and happiness
    Feed,
    /// Play with your pet to increase happiness
    Play,
    /// Walk your pet to manage potty needs (uses energy)
    Walk,
    /// Put your pet to sleep to restore energy
    Sleep,
    /// Train your pet to gain XP and level up
    Train,
    /// Help your pet go potty to reset potty level
    Potty,
    /// Clean your pet to increase cleanliness
    Clean,
    /// Reset your pet and start over
    Reset,
    /// Enter interactive shell mode
    Shell,
}

fn main() {
    let cli = Cli::parse();

    match cli.command {
        Commands::Adopt { name, species } => match adopt_pet(&name, &species) {
            Ok(message) => println!("{}", message),
            Err(e) => {
                eprintln!("Error: {}", e);
                std::process::exit(1);
            }
        },
        Commands::Status => match show_status() {
            Ok(_) => {}
            Err(e) => {
                eprintln!("Error: {}", e);
                std::process::exit(1);
            }
        },
        Commands::Feed => match feed_pet() {
            Ok(_) => {}
            Err(e) => {
                eprintln!("Error: {}", e);
                std::process::exit(1);
            }
        },
        Commands::Play => match play_pet() {
            Ok(_) => {}
            Err(e) => {
                eprintln!("Error: {}", e);
                std::process::exit(1);
            }
        },
        Commands::Walk => match walk_pet() {
            Ok(_) => {}
            Err(e) => {
                eprintln!("Error: {}", e);
                std::process::exit(1);
            }
        },
        Commands::Sleep => match sleep_pet() {
            Ok(_) => {}
            Err(e) => {
                eprintln!("Error: {}", e);
                std::process::exit(1);
            }
        },
        Commands::Train => match train_pet() {
            Ok(_) => {}
            Err(e) => {
                eprintln!("Error: {}", e);
                std::process::exit(1);
            }
        },
        Commands::Potty => match potty_pet() {
            Ok(_) => {}
            Err(e) => {
                eprintln!("Error: {}", e);
                std::process::exit(1);
            }
        },
        Commands::Clean => match clean_pet() {
            Ok(_) => {}
            Err(e) => {
                eprintln!("Error: {}", e);
                std::process::exit(1);
            }
        },
        Commands::Reset => match reset_pet() {
            Ok(_) => {}
            Err(e) => {
                eprintln!("Error: {}", e);
                std::process::exit(1);
            }
        },
        Commands::Shell => match run_shell() {
            Ok(_) => {}
            Err(e) => {
                eprintln!("Error: {}", e);
                std::process::exit(1);
            }
        },
    }
}
